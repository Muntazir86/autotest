...
from autotest.core import NotSet
from autotest.core.errors import IncorrectUsage
from autotest.core.rate_limit import ratelimit
from autotest.core.transforms import merge_at
from autotest.core.transport import DEFAULT_RESPONSE_TIMEOUT, Response
from autotest.generation.overrides import Override
from autotest.transport import BaseTransport, SerializationContext
from autotest.transport.prepare import get_exclude_headers, prepare_body, prepare_headers, prepare_url
from autotest.transport.serialization import Binary, serialize_binary, serialize_json, serialize_xml, serialize_yaml
...
def validate_vanilla_requests_kwargs(data: dict[str, Any]) -> None:
    ...
    if not urlparse(url).netloc:
        ...
        raise IncorrectUsage(
            "The `base_url` argument is required when specifying a schema via a file, so Autotest knows where to send the data. \n"
            f"Pass `base_url` either to the `autotest.openapi.from_*` loader or to the `Case.{method_name}`.\n"
...
def _prepare_form_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    If the schema is loose, Autotest can generate data that can't be sent as multipart. In these cases,
