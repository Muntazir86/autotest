...
from autotest.core.jsonschema import BUNDLE_STORAGE_KEY, get_type
from autotest.core.jsonschema.types import JsonSchemaObject
...
class MutationContext:
    ...
    def mutate(self, draw: Draw) -> tuple[Schema, MutationMetadata | None]:
        # On the top level, Autotest creates "object" schemas for all parameter "in" values except "body", which is
        ...
        if self.location.is_in_header:
            ...
            if self.allow_extra_parameters and draw(st.booleans()):
                # In headers, `additionalProperties` are False by default, which means that Autotest won't generate
