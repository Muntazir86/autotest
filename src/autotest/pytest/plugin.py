...
from autotest.core.compat import BaseExceptionGroup
from autotest.core.control import SkipTest
...
from autotest.core.failures import FailureGroup, get_origin
from autotest.core.marks import Mark
from autotest.core.result import Ok, Result
from autotest.generation import overrides
from autotest.generation.hypothesis.given import (
    GivenArgsMark,
    GivenKwargsMark,
    is_given_applied,
    merge_given_args,
...
from autotest.generation.hypothesis.reporting import (
    HealthCheckTipStyle,
    build_health_check_error,
    build_unsatisfiable_error,
    ignore_hypothesis_output,
)
from autotest.pytest.control_flow import fail_on_no_matches
...
class AutotestFunction(Function):
    def __init__(
        self,
        *args: Any,
        test_func: Callable,
...
class AutotestCase(PyCollector):
    ...
    def _gen_items(self, result: Result[APIOperation, InvalidSchema]) -> Generator[AutotestFunction, None, None]:
        ...
        """
        from autotest.checks import load_all_checks
        ...
        if not metafunc._calls:
            yield AutotestFunction.from_parent(
                name=name,
                parent=self.parent,
                callobj=funcobj,
...
def pytest_pycollect_makeitem(collector: nodes.Collector, name: str, obj: Any) -> Generator[None, Any, None]:
    """Switch to a different collector if the test is parametrized marked by Autotest."""
    outcome = yield
    try:
        schema = SchemaHandleMark.get(obj)
        assert schema is not None
        outcome.force_result(AutotestCase.from_parent(collector, test_function=obj, name=name, schema=schema))
    except Exception:
        outcome.get_result()
...
def pytest_runtest_logreport(report: pytest.TestReport) -> None:
    if isinstance(report, SubTestReport) and report.passed:
        report._Autotest_ignore_in_summary = True
...
def pytest_terminal_summary(terminalreporter: "TerminalReporter") -> Generator[None, None, None]:
    ...
    if passed:
        terminalreporter.stats["passed"] = [
            report for report in passed if not getattr(report, "_Autotest_ignore_in_summary", False)
        ]
    yield
...
def pytest_exception_interact(node: Function, call: pytest.CallInfo, report: pytest.TestReport) -> None:
    if call.excinfo:
        if issubclass(call.excinfo.type, AutotestError) and hasattr(call.excinfo.value, "__notes__"):
            # Hypothesis adds quite a lot of additional debug information which is not that helpful in Autotest
        ...
        if call.excinfo.type is FailureGroup:
            ...
            # Keep internal Autotest frames + one extra one from the caller
            skip_frames = 0
            for i in range(total_frames - 1, -1, -1):
                entry = tb_entries[i]

                if not str(entry.path).endswith("Autotest/generation/case.py"):
                    skip_frames = i
                    break
...
def pytest_pyfunc_call(pyfuncitem):  # type: ignore[no-untyped-def]
    ...
    if isinstance(pyfuncitem, AutotestFunction):
        try:
            with ignore_hypothesis_output():
                yield
        except InvalidArgument as exc:
            if "Inconsistent args" in str(exc) and "@example()" in str(exc):
                from autotest.generation.hypothesis.given import GIVEN_AND_EXPLICIT_EXAMPLE_ERROR_MESSAGE
