...
from autotest.config._operations import OperationConfig, OperationsConfig
from autotest.config._output import OutputConfig
from autotest.config._parameters import load_parameters
from autotest.config._phases import PhasesConfig
from autotest.config._rate_limit import build_limiter
from autotest.config._report import ReportsConfig
from autotest.config._warnings import WarningsConfig
from autotest.core import HYPOTHESIS_IN_MEMORY_DATABASE_IDENTIFIER, NOT_SET, NotSet, hooks
from autotest.core.validation import validate_base_url
...
class ProjectConfig(DiffBase):
    ...
    def __init__(
        self,
        *,
        parent: AutotestConfig | None = None,
    ...
    def _get_parent(self) -> AutotestConfig:
        if self._parent is None:
            from autotest.config import AutotestConfig

            self._parent = AutotestConfig.discover()
        return self._parent
...
class ProjectsConfig(DiffBase):
    ...
    def _set_parent(self, parent: AutotestConfig) -> None:
