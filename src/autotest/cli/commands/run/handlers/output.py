...
from autotest.cli.commands.run.context import ExecutionContext, GroupedFailures
...
from autotest.cli.constants import ISSUE_TRACKER_URL
from autotest.cli.core import get_terminal_width
from autotest.config import ProjectConfig, ReportFormat, AutotestWarning
from autotest.core.errors import LoaderError, LoaderErrorKind, format_exception, split_traceback
from autotest.core.failures import MessageBlock, Severity, format_failures
from autotest.core.output import prepare_response_payload
...
from autotest.engine.phases import PhaseName, PhaseSkipReason
from autotest.engine.phases.probes import ProbeOutcome
from autotest.engine.recorder import Interaction, ScenarioRecorder
from autotest.generation.meta import CoveragePhaseData, CoverageScenario
from autotest.generation.modes import GenerationMode
from autotest.schemas import ApiStatistic
...
if TYPE_CHECKING:
    ...
    from autotest.generation.stateful.state_machine import ExtractionFailure
...
LOADER_ERROR_SUGGESTIONS = {
    ...
    # Unclassified
    LoaderErrorKind.UNCLASSIFIED: f"If you suspect this is a Autotest issue and the schema is valid, please report it and include the schema if you can:\n\n  {ISSUE_TRACKER_URL}",
}
...
class OutputHandler(EventHandler):
    ...
    def _check_warnings(self, ctx: ExecutionContext, event: events.ScenarioFinished) -> None:
        ...
        if warnings.should_display(AutotestWarning.MISSING_AUTH):
            if not (len(event.recorder.cases) == 1 and has_only_missing_auth_case()):
                for status_code in (401, 403):
                    if statistic.ratio_for(status_code) >= AUTH_ERRORS_THRESHOLD:
                        ...
                        if warnings.should_fail(AutotestWarning.MISSING_AUTH):
                            ctx.exit_code = 1
        ...
        if (
            ...
            and (
                warnings.should_display(AutotestWarning.MISSING_TEST_DATA)
                or warnings.should_display(AutotestWarning.VALIDATION_MISMATCH)
            )
    ...
    def _on_schema_warnings(self, ctx: ExecutionContext, event: events.SchemaAnalysisWarnings) -> None:
        ...
        for warning in event.warnings:
            if warning.kind is AutotestWarning.MISSING_DESERIALIZER:
            ...
            elif warning.kind is AutotestWarning.UNUSED_OPENAPI_AUTH:
            ...
            elif warning.kind is AutotestWarning.UNSUPPORTED_REGEX:
                assert warning.operation_label is not None
                self._handle_warning(
                    ctx,
                    warning.kind,
    ...
    def display_warnings(self) -> None:
        ...
        if self.warnings.missing_deserializer:
            self._display_detailed_warning_block(
                ...
                tips=["💡 Register a deserializer with @autotest.deserializer() to enable validation"],
            )
