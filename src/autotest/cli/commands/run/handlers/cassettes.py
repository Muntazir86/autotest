...
from autotest.cli.commands.run.handlers.base import EventHandler, TextOutput, open_text_output
from autotest.config import ProjectConfig, ReportFormat, AutotestConfig
...
from autotest.engine.recorder import CheckNode, Request, ScenarioRecorder
from autotest.generation.meta import CoveragePhaseData
        ...
        while True:
            ...
            if isinstance(item, Process):
                ...
                stream.write(
                    ...
                    f"recorded_with: 'Autotest {AUTOTEST_VERSION}'\n"
                    f"seed: {item.seed}\n"
                    f"http_interactions:"
                )
            else:
...
def har_writer(output: TextOutput, config: AutotestConfig, queue: Queue) -> None:
