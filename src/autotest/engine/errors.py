"""Handling of recoverable errors in Autotest Engine.

This module provides utilities for analyzing, classifying, and formatting exceptions
that occur during test execution via Autotest Engine.
"""
...
class DeadlineExceeded(errors.AutotestError):
...
class UnexpectedError(errors.AutotestError):
...
def extract_health_check_error(error: hypothesis.errors.FailedHealthCheck) -> hypothesis.HealthCheck | None:
    from autotest.generation.hypothesis.reporting import HEALTH_CHECK_TITLES
...
def get_runtime_error_suggestion(error_type: RuntimeErrorKind, bold: Callable[[str], str] = str) -> str | None:
    ...
    from autotest.generation.hypothesis.reporting import HEALTH_CHECK_ACTIONS
    ...
    return {
        ...
        "For guidance, visit: https://Autotest.readthedocs.io/en/stable/guides/graphql-custom-scalars/",
...
def clear_hypothesis_notes(exc: Exception) -> None:
    ...
    # Only remove "You can reproduce" blocks (non-working for Autotest)
