"""A lightweight mechanism to attach Autotest-specific metadata to test functions."""
...
METADATA_ATTR = "_Autotest_metadata"


@dataclass
class AutotestMetadata:
    """Container for all Autotest-specific data attached to test functions."""
...
class Mark(Generic[T]):
    """Access to specific attributes in AutotestMetadata."""
    ...
    def set(self, func: Callable, value: T) -> None:
        ...
        if not hasattr(func, METADATA_ATTR):
            setattr(func, METADATA_ATTR, AutotestMetadata())
