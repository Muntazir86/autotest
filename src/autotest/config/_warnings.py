...
class AutotestWarning(str, enum.Enum):
    ...
    @classmethod
    def from_str(cls, value: str) -> AutotestWarning:
        return {
...
class WarningsConfig(DiffBase):
    ...
    display: list[AutotestWarning]
    """Which warnings to display in output."""

    fail_on: list[AutotestWarning]
    ...
    def __init__(
        self,
        *,
        display: list[AutotestWarning] | None = None,
        fail_on: list[AutotestWarning] | None = None,
    ) -> None:
        self.display = display if display is not None else list(AutotestWarning)
    ...
    def from_value(cls, value: bool | list[str] | dict[str, Any] | None) -> WarningsConfig:
        ...
        if value is None or value is True:
            ...
            warnings = [AutotestWarning.from_str(resolve(w)) for w in value]
        ...
        if not enabled:
            ...
            display = [AutotestWarning.from_str(resolve(w)) for w in display_list]
        else:
            display = list(AutotestWarning)
        ...
        if fail_on is False or fail_on is None:
            ...
            fail_on_list = [AutotestWarning.from_str(resolve(w)) for w in fail_on]
        else:
            fail_on_list = []

        return cls(display=display, fail_on=fail_on_list)

    def should_display(self, warning: AutotestWarning) -> bool:
        """Check if a warning should be displayed."""
        return warning in self.display

    def should_fail(self, warning: AutotestWarning) -> bool:
