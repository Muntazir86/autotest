...
Autotest sends specially crafted requests to the application before running tests in order to detect whether
...
if TYPE_CHECKING:
    ...
    from autotest.engine.events import EventGenerator
...
def execute(ctx: EngineContext, phase: Phase) -> EventGenerator:
    ...
    for result in probes:
        if isinstance(result.probe, NullByteInHeader) and result.is_failure:
            from autotest.specs.openapi import formats
            from autotest.specs.openapi.formats import (
...
class NullByteInHeader(Probe):
    ...
    def prepare_request(
        ...
        request.headers = {"x-autotest-Probe-Null": "\x00"}
