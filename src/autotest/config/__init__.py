...
from autotest.config._auth import ApiKeyAuthConfig, HttpBasicAuthConfig, HttpBearerAuthConfig
from autotest.config._checks import (
    CheckConfig,
    ChecksConfig,
...
from autotest.config._generation import GenerationConfig
from autotest.config._health_check import HealthCheck
from autotest.config._output import OutputConfig, SanitizationConfig, TruncationConfig
from autotest.config._phases import (
    CoveragePhaseConfig,
    ExamplesPhaseConfig,
...
from autotest.config._projects import ProjectConfig, ProjectsConfig, get_workers_count
from autotest.config._report import DEFAULT_REPORT_DIRECTORY, ReportConfig, ReportFormat, ReportsConfig, HTMLReportConfig
from autotest.config._extraction import IDExtractionConfig
from autotest.config._warnings import AutotestWarning, WarningsConfig
...
class AutotestConfig(DiffBase):
    ...
    @classmethod
    def discover(cls) -> AutotestConfig:
        """Discover the Autotest configuration file.

        Search for 'autotest.toml' in the current directory and then in each parent directory,
        ...
        while True:
            candidate = os.path.join(current_dir, "autotest.toml")
    ...
    @classmethod
    def from_path(cls, path: PathLike | str) -> AutotestConfig:
    ...
    @classmethod
    def from_str(cls, data: str) -> AutotestConfig:
        """Parse configuration from a string."""
        parsed = tomli.loads(data)
        return cls.from_dict(parsed)

    @classmethod
    def from_dict(cls, data: dict) -> AutotestConfig:
        """Create a config instance from a dictionary."""
        from jsonschema.exceptions import ValidationError

        from autotest.config._validator import CONFIG_VALIDATOR
