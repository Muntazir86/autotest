...
from autotest.core.output import truncate_json
...
if TYPE_CHECKING:
    ...
    from autotest.config import OutputConfig
...
class AutotestError(Exception):
    """Base exception class for all Autotest errors."""
...
class InvalidSchema(AutotestError):
...
class RemoteDocumentError(AutotestError):
...
class HookError(AutotestError):
    ...
    def __str__(self) -> str:
        return f"Failed to load Autotest extensions from `{self.module_path}`"
...
class InvalidStateMachine(AutotestError):
...
class InvalidTransition(AutotestError):
...
class TransitionValidationError(AutotestError):
...
class IncorrectUsage(AutotestError):
    """Indicates incorrect usage of Autotest' public API."""
...
class InternalError(AutotestError):
    """Internal error in Autotest."""


class SerializationError(AutotestError):
...
class UnresolvableReference(AutotestError):
...
class InfiniteRecursiveReference(AutotestError):
...
class OperationNotFound(LookupError, AutotestError):
...
class LoaderError(AutotestError):
