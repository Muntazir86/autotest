...
from autotest.checks import CHECKS, CheckContext, CheckFunction, load_all_checks, run_checks
from autotest.core import NOT_SET, AUTOTEST_TEST_CASE_HEADER, NotSet, curl
from autotest.core.errors import IncorrectUsage
from autotest.core.failures import FailureGroup, failure_report_title, format_failures
from autotest.core.parameters import CONTAINER_TO_LOCATION, ParameterLocation
from autotest.core.transport import Response
from autotest.generation import GenerationMode, generate_random_case_id
from autotest.generation.meta import CaseMetadata, ComponentInfo
from autotest.generation.overrides import Override, store_components
from autotest.hooks import HookContext, dispatch
from autotest.transport.prepare import prepare_path, prepare_request
...
class Case:
    ...
    def __hash__(self) -> int:
        return hash(self.as_curl_command({AUTOTEST_TEST_CASE_HEADER: "0"}))
    ...
    def validate_response(
        ...
        # For example - non-Autotest tests that manually construct `Case` instances
        load_all_checks()
