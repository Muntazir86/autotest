...
from autotest.generation.meta import CoveragePhaseData, CoverageScenario, FuzzingPhaseData, StatefulPhaseData
...
def prepare_headers(case: Case, headers: dict[str, str] | None = None) -> CaseInsensitiveDict:
    ...
    default_headers.setdefault(AUTOTEST_TEST_CASE_HEADER, case.id)
    if headers:
