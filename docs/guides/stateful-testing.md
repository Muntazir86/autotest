...
This guide shows how to customize Autotest's stateful testing to handle non-standard authentication scenarios, inject realistic test data, and adapt requests for your environment.
...
Stateful testing only runs when Autotest can discover relationships between your operations.

This can happen in three ways:

- **Automatic analysis.** Autotest infers links from your response schemas and path parameters.
...
!!! info "How is it implemented?"
    Autotest implements stateful testing on top of Hypothesis's [rule-based state machines](https://hypothesis.readthedocs.io/en/latest/stateful.html)
...
Autotest extends link resolution with regex extraction. This is handy when responses return a `Location` header:
...
If `autotest run` still reports `Missing Open API links`:

1. Review the stateful phase summary (`API Links: ...`) to see how many links were discovered or filtered out.
2. Fix any extraction errors Autotest reports (invalid expressions, missing parameters) and rerun.
...
def api_schema(database, app_config):
    ...
    return autotest.openapi.from_url("http://localhost:8000/openapi.json")
...
Stateful testing is enabled by default - `autotest run http://localhost:8000/openapi.yaml` executes examples, coverage, fuzzing, and finally the stateful phase. The summary (`API Links: ...`) shows whether any links were executed.

If no links are available, Autotest skips the stateful phase in a default run. When you run only the stateful phase (`--phases=stateful`) and no links exist, it reports `Missing Open API links`. Provide the necessary links or disable the phase explicitly.

Disabling earlier phases via `--phases` keeps dependency analysis active, but Autotest cannot learn additional links from observed `Location` headers. Manual links remain fully supported either way.
